...
object Client extends App {
  ...
  val akkaServerlessHost = "sparkling-wildflower-0239.us-east1.apps.akkaserverless.io"


  var channel = NettyChannelBuilder.forAddress(akkaServerlessHost, 443).sslContext(GrpcSslContexts.forClient().build()).build()
  ...
  val traders = Future.sequence {
    List(
      createTraders(25, AssetPairs.`BTC/EUR`, "exponential").map(AssetPairs.`BTC/EUR` -> _),
      createTraders(25, AssetPairs.`BTC/USDT`, "simple").map(AssetPairs.`BTC/USDT` -> _),
      createTraders(25, AssetPairs.`ETH/USDT`, "simple").map(AssetPairs.`ETH/USDT` -> _),
      createTraders(25, AssetPairs.`ETH/EUR`, "simple").map(AssetPairs.`ETH/EUR` -> _)
    )
  ...
  traders.flatMap { assetPairsToTraders =>
    Future.sequence(assetPairsToTraders.map {
      case (assetPair, traders) => sendHistoricalCandles(assetPair, traders).flatMap { _ =>
        sendLiveCandles(assetPair, traders)._1
      }
    })
  }

  Source.tick(1.second, 15.second, Done)
    .mapConcat(_ => List("BTC", "ETH").map(base => ByBaseAsset.newBuilder().setBaseAsset(base).build()))
    .mapAsyncUnordered(8) { byAsset =>
      Future(tradingViewsClient.getTraders(byAsset))
    }.mapConcat(_.asScala)
    .map { state =>

      val list = List[String](state.getBaseAsset, fixDigits(state.getBaseBalance), state.getQuoteAsset, fixDigits(state.getQuoteBalance))
  ...
  def createTraders(n: Int, assetPair: AssetPair, movingAverageType: String): Future[Seq[String]] = {
    Source(List.fill(n) {
      ...
      val shortMaPeriod = 15 + Random.nextInt(15)
      val longMaPeriod = 30 + Random.nextInt(90)
      val randomId = 1000 + Random.nextInt(1000000)
      val threshold = Math.random()
      val traderId = s"${assetPair._1}/${assetPair._2}_1m_${movingAverageType}_${shortMaPeriod}_${longMaPeriod}_${threshold}_${randomId}"
      NewTraderCommand.newBuilder()
        ...
        .setTrader(TraderState.newBuilder()
          ...
          .setShortMaValue(shortMaPeriod)
          .setLongMaValue(longMaPeriod)
          .setBaseBalance(10)
  ...
  def sendHistoricalCandles(assetPair: AssetPair, traders: List[String]) = {
    getHistoricalCandles(Instant.now().minus(90, ChronoUnit.DAYS), 1.minutes, assetPair)
      .mapConcat { candle =>
        traders.map(traderId => AddCandleCommand.newBuilder().setCandle(candle).setTraderId(traderId).build())
      }
      .mapAsyncUnordered(8) { command =>
